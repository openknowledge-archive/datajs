/* Given the json from a google docs spreadsheet convert it to tabular format.
...
*/
function gdocsToTabular(gdocsSpreadsheetJson) {
	var options = {};
	...
	if (options.columnsToUse ) {
		...
		if (gdocsSpreadsheetJson.feed.entry.length > 0) {
			for (var k in gdocsSpreadsheetJson.feed.entry[0]) {
				if (k.substr(0,3) == 'gsx') {
					var col=k.substr(4)
	...
	var rep = /^([\d\.\-]+)\%$/;
	$.each(gdocsSpreadsheetJson.feed.entry, function(i,entry) {
		var row = [];
